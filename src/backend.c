 ...
 * is found, NULL is returned.
 */
struct server *get_server_hashon(struct session *s)
{
    struct http_txn *txn  = &s->txn;
    struct proxy *px   = s->be;
    struct hdr_ctx ctx;
    ...
    if (matched_rule) {
        key = sample_fetch_string(px, s, txn, SMP_OPT_DIR_REQ|SMP_OPT_FINAL, matched_rule->expr);
    }
    ...
    if (key) {
        unsigned int hash = gen_hash(px, key->data.str.str, key->data.str.len);
        if ((px->lbprm.algo & BE_LB_HASH_MOD) == BE_LB_HMOD_AVAL)
            hash = full_hash(hash);
        if (px->lbprm.algo & BE_LB_LKUP_CHTREE)
